...
function get_media($type,$id){
 ...
 @$conn = dbconnect('admin','PDO');
  ...
  $sql = "SELECT pm_link_name,pm_extension FROM patient_media_table WHERE p_id = :id";
  $stmt = $conn->prepare($sql);
  $stmt->bindParam(':id',$id,PDO::PARAM_INT);
  $stmt->bindColumn('pm_link_name',$link_name);
  $stmt->bindColumn('pm_extension',$extension);
 }elseif($type === 'doctor'){
  $sql = "SELECT dm_link_name,dm_extension FROM doctor_media_table WHERE d_id = :id";
  $stmt = $conn->prepare($sql);
  $stmt->bindParam(':id',$id,PDO::PARAM_INT);
  $stmt->bindColumn('dm_link_name',$link_name);
  $stmt->bindColumn('dm_extension',$extension);
 }else{
  return 'home/no_media.png';
   ...
   if(file_exists(file_location('media_path',$file)) && is_file(file_location('media_path',$file))){
    return $file;
     ...
     if($type === 'admin' || $type === 'patient'|| $type === 'doctor'){return 'home/avatar.png';}else{return 'home/no_media.png';}
   }
		}
	}else{
  if($type === 'admin' || $type === 'patient'|| $type === 'doctor'){return 'home/avatar.png';}else{return 'home/no_media.png';}
