...
if(isset($_POST['t'])){
	...
	$ty = ($_POST['t']);
	if($ty !== 'patient'){$missing[] = "t";}else{$type = test_input($ty);}
	
	if(empty($error) and empty($missing)){
		//FORM IMAGE UPLOAD
		$location = $type;$size = 8000000;$file_mode = ["image/png","image/jpeg"];$file_type = 'image';$input_name = "{$type}_pics";$file_name = ucwords($type)."_".time_token();
		...
		if(empty($missing) && empty($error)){
			if($file2 === "larger"){ // if file is larger tha expected echo error
				...
				if($type === 'patient'){
					...
					$patient->file_name = $correct['filename'];
					$patient->extension = $correct['extension'];
					$change = $patient->change_image($type);
				}else{
					$missing[] = 'missing';
				}
