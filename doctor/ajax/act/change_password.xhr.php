...
if(isset($_POST["opss"]) && isset($_POST["npss"])){
	...
	if(!password_verify($old_pass,content_data('patient_table','p_password',$p_id,'p_id'))){
	...
	if(empty($error)){
		...
		$patient->col = "p_password";
		$patient->data = $newpass;
		$change = $patient->update_patient_data();
		if($change === 'success'){
