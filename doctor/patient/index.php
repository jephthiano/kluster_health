			...
			<div id=""class="j-col m10">
				...
				<div class="j-main-body">
					<div class='j-color4'style="width:100%;max-width:800px">
						<?php $header="Patients ({$patient_counter})";$back="hide"; get_header($header,$back,'','','','j-color9');?>
						<?php
						...
						if($status === 'pending'){
							...
							$p_idor = multiple_content_data('guidance_request_table','p_id','accepted','gr_status',"AND d_id = {$d_id}",'unique');
							if($p_idor === false){
								?><div class="j-center"><br><br>You have no patient to monitor at the moment</div><?php
							}else{
								?>
								<div class="j-row-padding">
									<?php foreach($p_idor AS $p_id){patient_section_data('profile',$p_id);}?>
								</div>
								<?php
							}
						}
