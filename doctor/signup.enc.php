	...
	<div class="j-round j-panel j-border j-border-color5"style="width:100%;max-width:400px;height:auto;">
		...
		<center>
			<span class="j-text-color9 j-large"style=''>
				...
				<b class='j-bolder j-xlarge j-text-color7'>DOCTOR SIGN UP</b>
			</span><br>
		</center>
		<form id='dsgfrm'method='post'>
			<br>
			<?php
			get_form_type('email','ema','sgpbtn','Email','','1','70');//for email input
			get_form_type('text','fnm','sgpbtn','Fullname','','3','30');//for fullname input
			get_form_type('number','rgid','sgpbtn','Med Certificate Number','','1','70');//for email input
			get_form_password('sgpbtn');//for password input
			get_form_button('sgpbtn','Sign Up','')// submit button
			?>
		</form>
		<br>
		<center><a class="j-text-color3 j-center j-bolder"href="<?= file_location('doctor_url','login/');?>">Already a member? Login In</a></center>
		<br><br><br>
	</div>
	</center>
	<span id='st'></span>
